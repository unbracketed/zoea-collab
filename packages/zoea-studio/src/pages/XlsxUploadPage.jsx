...
function XlsxUploadPage() {
  ...
  const handleUpload = async () => {
    if (!file) {
      setStatus('Please select a spreadsheet to upload.')
      return
    }
    ...
    setIsUploading(true)
    setStatus('Uploading spreadsheet...')
    try {
      await api.createXlsxDocument({
        name: file.name,
        description,
  ...
  return (
    <LayoutFrame title="Upload Spreadsheet" variant="content-centered">
      <div className="max-w-4xl mx-auto py-4 px-4">
        <div className="mb-6">
          <h1 className="text-2xl font-bold">Upload Spreadsheet</h1>
          <p className="text-text-secondary">Upload an Excel spreadsheet (.xlsx) for viewing and data extraction.</p>
        </div>
        ...
        <div className="max-w-2xl">
          <div className="bg-surface rounded-lg shadow-soft border border-border">
            <div className="p-6">
              <div className="mb-4">
                <label className="block text-sm font-medium mb-1">Spreadsheet File</label>
