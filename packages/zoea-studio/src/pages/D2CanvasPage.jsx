/**
 * D2 Canvas Page - D2 Diagram Playground
 *
 ...
 *
 * This is the dedicated D2 canvas at /canvas/d2, preserved for D2-specific workflows.
 */
...
const LOCALSTORAGE_KEY_PREFIX = 'zoea-d2canvas-code';
...
function D2CanvasPage() {
  ...
  const handleSaveToClipboard = async () => {
    if (!currentWorkspaceId) {
      setClipboardStatus('Select a workspace to save this diagram to your notebook.');
      return;
    }

    if (!trimmedCode) {
      setClipboardStatus('Add diagram code before saving to your notebook.');
      return;
    }

    setIsSavingClipboard(true);
    setClipboardStatus('Saving to notebook...');
    try {
      ...
      setClipboardStatus('Saved to notebook.');
    } catch (error) {
      setClipboardStatus(error.message || 'Failed to save to notebook.');
    } finally {
    ...
    <div className="flex gap-2">
      {trimmedCode && (
        <button className="px-3 py-1.5 text-sm border border-primary text-primary rounded hover:bg-primary hover:text-white transition-colors" onClick={handleSaveToClipboard} disabled={isSavingClipboard}>
          {isSavingClipboard ? 'Saving…' : 'Save to Notebook'}
        </button>
      )}
