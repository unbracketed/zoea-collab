"""Notepad (Yoopta) helpers for Clipboards.

Notepads are a UI concept: the canonical data model remains Clipboard + ClipboardItem.
For V1, we support exporting a clipboard to shareable Yoopta JSON without requiring
...
YOOPTA_CLIPBOARD_ITEM_BLOCK_TYPE = "ClipboardItem"
YOOPTA_CLIPBOARD_ITEM_ELEMENT_TYPE = "clipboard_item"


def get_notepad_draft_content(clipboard: Clipboard) -> dict[str, Any] | None:
    """Return the stored Yoopta JSON dict from Clipboard.metadata (if present)."""
...
def build_shareable_yoopta_content_for_clipboard(clipboard: Clipboard) -> dict[str, Any]:
    """Build shareable Yoopta JSON content for a clipboard.

    - If a notepad draft exists in clipboard.metadata, use it as the base.
    - Otherwise, synthesize Yoopta JSON from the clipboard items.

    This function intentionally produces content that does not depend on private
    ClipboardItem IDs to render (suitable for saving as a shared YooptaDocument).
    """

    draft = get_notepad_draft_content(clipboard)
    if draft:
        return _sanitize_for_export(clipboard, draft)

    return _build_yoopta_content_from_items(clipboard)


def _sanitize_for_export(clipboard: Clipboard, content: dict[str, Any]) -> dict[str, Any]:
    """Best-effort sanitize/export normalization for Yoopta JSON.

    Notepad drafts can include private ClipboardItem references (Yoopta embed blocks).
    When exporting to a shared YooptaDocument, we replace embedded ClipboardItem blocks
    ...
    for block in ordered_blocks:
        item_id = _extract_clipboard_item_id(block)
    ...
    items = list(
        clipboard.items.select_related("content_type", "virtual_node")
    ...
    return {block["id"]: block for block in sanitized_blocks if isinstance(block, dict) and block.get("id")}


def _extract_clipboard_item_id(block: dict[str, Any]) -> int | None:
    if block.get("type") != YOOPTA_CLIPBOARD_ITEM_BLOCK_TYPE:
        return None
    ...
    def _scan(node: Any) -> int | None:
        if not isinstance(node, dict):
            return None
        if node.get("type") == YOOPTA_CLIPBOARD_ITEM_ELEMENT_TYPE:
            props = node.get("props")
            if isinstance(props, dict):
                raw = props.get("clipboard_item_id")
                if raw is None:
                    return None
                try:
                    return int(raw)
...
def _shareable_blocks_for_item(
    *,
    ...
    if item is None:
        return [_paragraph_block(order=start_order, text="[Clipboard item]")]
    ...
    full_text = (item.source_metadata or {}).get("full_text") or (item.source_metadata or {}).get("preview")
    if full_text:
...
def _build_yoopta_content_from_items(clipboard: Clipboard) -> dict[str, Any]:
    items = list(
        clipboard.items.select_related("content_type", "virtual_node")
    ...
    blocks.append(
        _heading_block(
            order=0,
            text=clipboard.name or "Notepad",
            level=1,
        )
    )
    ...
    for item in items:
        ...
        full_text = (item.source_metadata or {}).get("full_text") or (item.source_metadata or {}).get(
            "preview"
        )
        if full_text:
